...
beforeAll(async () => {
  ...
  const disc = await discover(s, { provider: new MockLlmProvider(readSavingsScript()), surface, safety: gate(), allowlist: { routes: ["http://localhost:4010/**"], actions: [...ALL] }, trace: new TraceWriter(makeRunId("discovery", s.capabilityId), "discovery", dir) });
